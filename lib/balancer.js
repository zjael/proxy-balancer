...
class Balancer {
  constructor(config) {
    ...
    this.ipLimiter = (config.callsPerDuration && config.duration && config.postDurationWait)
      && new RateLimiterMemory({ points: config.callsPerDuration, duration: config.duration / 1000 });
  ...
  async request(url, options, timeout = this.config.timeout / 1000, { retryCount = 0, timesThisIpRetried = 0, ipsTried = 1 } = {}) {
    try {
      ...
      // TODO: IF IT FAILS AND THERE IS A LIMITER WE NEED TO POSTPONE THIS PROXY FOR 5 MIN
