...
class Balancer {
  ...
  async nextProxyIndex(proxies) {
    if (this.ipLimiter) {
      ...
      const nextProxyAvailable = proxies.slice(this.currentProxy + 1, proxies.length).findIndex(async ip => {
        const url = this.formatProxy(ip)
        const limit = await this.ipLimiter.get(url)
        if (hasCallsRemaining(limit)) {
          if (limit && limit.remainingPoints === 1) {
      ...
      if (nextProxyAvailable > -1) {
        ...
        if (previousProxyAvailable > -1) {
          ...
          if (hasCallsRemaining(thisProxyLimit)) {
            if (thisProxyLimit && thisProxyLimit.remainingPoints === 1) {
              await this.blockAndResetProxy(url)
            }
          ...
          return null
          // need to check if can reuse the same proxy
        }
      }
    } else {
