...
class Balancer {
    ...
    try {
      ...
      if (this.ipLimiter) {
        ...
        try {
          ...
          throw new Error('No more valid IPs.')
        }
      }
  ...
  async blockAndResetProxy(url) {
    if (this.ipLimiter) {
      // block calls for duration
      await this.ipLimiter.block(url, this.config.postDurationWait)
      // set call count back to 0
      await this.ipLimiter.set(url, 0)
    }
  }

  async setNextProxy(proxies) {
    if (this.ipLimiter) {
      // find next available in line
      const nextProxyAvailable = proxies.slice(this.currentProxy, proxies.length).findIndex(async ip => {
        const url = this.formatProxy(ip)
        const limit = await this.ipLimiter.get(url)
        if (!limit || (limit && limit.remainingPoints > 0)) {
          if (limit.remainingPoints === 1) {
            await this.blockAndResetProxy(url)
          }
          return true
        }
        return false
      ...
      if (nextProxyAvailable > -1) {
        this.currentProxy = nextProxyAvailable
      } else {
        ...
        if (previousProxyAvailable > -1) {
          this.currentProxy = previousProxyAvailable
        } else {
      ...
      this.currentProxy = this.currentProxy + 1;
    }
  }
  ...
  async getNext() {
    ...
    await this.setNextProxy(proxies)
    let nextProxy = proxies[this.currentProxy];
    if (!nextProxy) {
      this.currentProxy = 0;
      nextProxy = proxies[0]
    }
